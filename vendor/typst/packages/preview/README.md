...
`profile-cv build` copies these packages into `~/.cache/typst/packages/preview` before invoking RenderCV because the embedded Typst compiler used by `typst-py` resolves preview packages from that cache path.
