...
- Keep `resume.yaml`, templates, tests, styles, workflow configuration, and
  maintainer documentation as the tracked source of truth.
...
- `resume.yaml` is canonical. Generated JSON, Markdown, DOCX, PDF, Typst, HTML,
  and site output are derived artifacts.
...
- Keep custom gates narrow. Use standard tools for syntax, formatting, typing,
  tests, workflow linting, and secret detection when available.
...
- Use targeted tests for behavior changes, then run the relevant full gate set
  ...
  Ruff, mypy, pytest, public-surface checks, workflow checks, README lint, and
  artifact QA where local system tools are available.
